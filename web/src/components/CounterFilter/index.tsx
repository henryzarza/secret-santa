...
interface Props {
  ...
  isActive?: boolean;
  isDisabled?: boolean;
  handleSelected: (status: string | null) => void;
}
...
const CountFilter = ({ quantity, status, text, isActive, handleSelected, isDisabled }: Props) => {
  return (
    <div className={`relative w-fit ${isDisabled && !isActive ? 'opacity-50 scale-90' : ''}`}>
      <button
      ...
      {isActive && (
        <button
          className="absolute right-3 -top-2 font-sans uppercase text-white text-sm font-bold px-4 py-1 gap-[10px] center bg-black bg-opacity-70 rounded-[50px] transition-colors hover:bg-opacity-85"
